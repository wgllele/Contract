...
import "./MultiSig.sol";
contract MultiSigColdPro is IMultiSigColdPro,MultiSig{
    constructor() {
    }
    /// @dev 资金平衡间隔时间
    ...
    //多签设置balancedTime，类似MultiSig.setManagerRatio,更多信息参考MultiSig.setManagerRatio
    ...
    //多签设置primAgentProfitPerc和feeRateToUtcPay，类似MultiSig.setManagerRatio,更多信息参考MultiSig.setManagerRatio
    function setAgentRateAndFeeRate(uint agentRate,uint feeRate,bytes memory signatures) external {
        {
            //TODO 参数feeRate需要判断不能低于默认值
            getBaseFeeRate(feeRate);
            bytes32 dataHash=_getTransactionHash(abi.encodeWithSelector(this.setAgentRateAndFeeRate.selector,
                agentRate,feeRate,new bytes(0)));
            ManagerInfo memory _managerInfo=managerInfo;
            uint managerNumber= _managerInfo.managerNumber;
            uint expirationTime= block.timestamp-_managerInfo.expirationTime;
            address[] memory managers=_managerInfo.managers;
    ...
    //多签设置multiSigAgentAddr，类似MultiSig.setManagerRatio,更多信息参考MultiSig.setManagerRatio
